...
CREATE TABLE album (
	albumNum INT NOT NULL PRIMARY KEY,
    title VARCHAR(30) NOT NULL,
    artist VARCHAR(30),
    price INT,
    from_date DATETIME,
    distributor VARCHAR(30),
    starScore DECIMAL(4,1)
);
-- 곡 테이블 설계
CREATE TABLE 곡 (
	앨범번호 INT NOT NULL,
...
CREATE TABLE song (
	albumNum INT NOT NULL,
    diskNum INT NOT NULL,
...
ALTER TABLE song
	ADD CONSTRAINT
	FOREIGN KEY (albumNum)
    REFERENCES album(albumNum);
...
-- mysql 쿼리  >> 간단하지만 가독성이 떨어진다.
SELECT 타이틀, 아티스트
FROM 앨범, 곡 as b
where 앨범.앨범번호 = b.앨범번호
and b.곡명 = '해변의 여인';
...
SELECT 타이틀, COUNT(곡명) "수록곡 개수"
FROM 앨범,
	(SELECT 앨범번호, 곡명 FROM 곡 WHERE 곡명 LIKE '%사랑%') AS b
WHERE 앨범.앨범번호 = b.앨범번호
GROUP BY 타이틀;
...
SELECT DISTINCT 타이틀
FROM 앨범
WHERE 앨범.타이틀 = 앨범.아티스트;


-- 문제 8 동일한 곡명이 2개 이상 앨범에 존재하는 경우,
-- 해당 곡명과 수록 곡의 개수를 검색하라.
SELECT COUNT(곡.곡명) "수록곡 개수", a.곡명, a.앨범번호
FROM 곡
INNER JOIN (SELECT 앨범번호, 곡명
			FROM 곡
			GROUP BY 곡명
			HAVING COUNT(곡명) >= 2) AS a
ON 곡.앨범번호 = a.앨범번호
GROUP BY 곡.앨범번호
